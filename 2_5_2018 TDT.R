...
ids_with_mend_errors<-read.csv("/users/lgai/ids_with_mend_errors.txt",header=FALSE)
...
chr8_ped<-read.csv("clean_pedfile_chr8_FILTERED.csv")
head(chr8_ped)
...
#Read in vcf file; takes a minute
filepath_vcf<-"/users/jli/gmkf.dup.vcf"
chr8_vcf <- readVcf(filepath_vcf,"hg19")
geno(chr8_vcf)$GT[geno(chr8_vcf)$GT == "0/."]<-"."
geno(chr8_vcf)$GT[geno(chr8_vcf)$GT == "1/."]<-"."

#Read in pedfile
filepath_ped<-"/users/jli/gmkf.dup.ped.csv"
...
vcf_ids_path<-file.path("/users/jli/gmkf.dup.id.txt")
...
chr8_geno <- vcf2geno(chr8_vcf,chr8_ped)
#Warning message:
#In vcf2geno(chr8_vcf, chr8_ped) :
#  Since NA of the SNVs were monomorphic, these SNVs were removed.



gtdt.gmkf.dup <- colTDT(chr8_geno)

save(gtdt.gmkf.dup, chr8_geno, file = "gtdt.gmkf.dup.RData")
