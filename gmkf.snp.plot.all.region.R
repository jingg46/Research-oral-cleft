...
load("/users/jli/gmkf.gTDT.RData")
tdtEuro <- gmkf.gtdt

## change rs to position
gmkf.rs.position <- read.csv("gmkf.rs.position.csv")
inx = which(names(tdtEuro$pval) %in% gmkf.rs.position$gmkf.rs.name)
for (i in 1:length(inx)){
  names(tdtEuro$pval)[inx[i]] <- gmkf.rs.position$gmkf.position[which(gmkf.rs.position$gmkf.rs.name %in% names(tdtEuro$pval)[inx[i]])]
}
...
write.table(tdtEuroSNPFile, file="/users/jli/tdtEuroSNPFile.gmkf.all.region.txt", sep="\t", row.names=FALSE, quote=FALSE)

###count # of variants
tdtEuroSNPFile <- read.table("/users/jli/tdtEuroSNPFile.gmkf.all.region.txt", header = T)
str(tdtEuroSNPFile)   
...
toWriteEuro <- makeGenotypeFile(gmkf.geno.filter)

write(toWriteEuro, file="/users/jli/tdtEuroDATFile.gmkf.all.region.dat")
...
snp.plotter(EVEN.SPACED = FALSE, PVAL.THRESHOLD = 1, USE.GBL.PVAL = TRUE,
            ...
            DISP.TYPE = "symbol", DISP.MULT.LAB.X = FALSE, DISP.SNP.NAMES = FALSE,
            DISP.CONNECTING.LINES = FALSE, LD.TYPE = "rsquare",
            LD.COLOR.SCHEME = "heat", USE.COLORS = TRUE, COLOR.LIST = "black",
            PALETTE.FILE = NULL, IMAGE.TITLE = NULL, IMAGE.NAME = "/users/jli/gmkf.all.region.snp.plot",
            IMAGE.TYPE = "pdf", IMAGE.SIZE = 3.5, CONNECTING.LINES.FACTOR = 1,
            CONNECTING.LINES.ADJ = 0, CONNECTING.LINES.VERT.ADJ = -1,
            CONNECTING.LINES.FLEX = 0, SNP.FILE = "/users/jli/tdtEuroSNPFile.gmkf.all.region.txt", HAP.FILE = NULL,
            GENOTYPE.FILE = "/users/jli/tdtEuroDATFile.gmkf.all.region.dat", FONT.FACTOR = NULL, SYMBOL.FACTOR = NULL,
            config.file = NULL)
