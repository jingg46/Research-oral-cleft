...
write.table(tdtEuroSNPFile, file="/users/jli/tdtEuroSNPFile.target.no.dup.all.region.txt", sep="\t", row.names=FALSE, quote=FALSE)

###count # of variants
tdtEuroSNPFile <- read.table("/users/jli/tdtEuroSNPFile.target.no.dup.all.region.txt", header = T)
str(tdtEuroSNPFile)  
...
write(toWriteEuro, file="/users/jli/tdtEuroDATFile.target.no.dup.all.region.dat")

library(snp.plotter)
snp.plotter(EVEN.SPACED = FALSE, PVAL.THRESHOLD = 1, USE.GBL.PVAL = TRUE,
            SYMBOLS = NA, SAMPLE.LABELS = NULL, LAB.Y = "log", DISP.HAP = FALSE,
            DISP.SNP = TRUE, DISP.COLOR.BAR = TRUE, DISP.PHYS.DIST = FALSE,
            ...
            PALETTE.FILE = NULL, IMAGE.TITLE = NULL, IMAGE.NAME = "/users/jli/target.no.dup.all.region.snp.plot",
            IMAGE.TYPE = "pdf", IMAGE.SIZE = 3.5, CONNECTING.LINES.FACTOR = 1,
            CONNECTING.LINES.ADJ = 0, CONNECTING.LINES.VERT.ADJ = -1,
            CONNECTING.LINES.FLEX = 0, SNP.FILE = "/users/jli/tdtEuroSNPFile.target.no.dup.all.region.txt", HAP.FILE = NULL,
            GENOTYPE.FILE = "/users/jli/tdtEuroDATFile.target.no.dup.all.region.dat", FONT.FACTOR = NULL, SYMBOL.FACTOR = NULL,
            config.file = NULL)
