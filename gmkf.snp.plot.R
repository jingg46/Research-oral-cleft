...
write.table(subset(tdtEuroSNPFile, LOC>=start & LOC <= end), file="/users/jli/tdtEuroSNPFile.gmkf.txt", sep="\t", row.names=FALSE, quote=FALSE)

###count # of variants
tdtEuroSNPFile <- read.table("/users/jli/tdtEuroSNPFile.gmkf.txt")
str(tdtEuroSNPFile)   ###276 variants
...
write(toWriteEuro, file="/users/jli/tdtEuroDATFile.gmkf.dat")
...
snp.plotter(EVEN.SPACED = FALSE, PVAL.THRESHOLD = 1, USE.GBL.PVAL = TRUE,
            ...
            PALETTE.FILE = NULL, IMAGE.TITLE = NULL, IMAGE.NAME = "/users/jli/gmkf.snp.plot",
            IMAGE.TYPE = "pdf", IMAGE.SIZE = 3.5, CONNECTING.LINES.FACTOR = 1,
            CONNECTING.LINES.ADJ = 0, CONNECTING.LINES.VERT.ADJ = -1,
            CONNECTING.LINES.FLEX = 0, SNP.FILE = "/users/jli/tdtEuroSNPFile.gmkf.txt", HAP.FILE = NULL,
            GENOTYPE.FILE = "/users/jli/tdtEuroDATFile.gmkf.dat", FONT.FACTOR = NULL, SYMBOL.FACTOR = NULL,
            config.file = NULL)
